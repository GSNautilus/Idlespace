...
**Version pinned:** `v0.1.4` (v0.2.0 fleet & combat rework largely landed; economy sim, idle tuning, decks & new cards still to come)
...
- The whole simulation advances on a **real wall-clock timer** (`pumpSim`), not on rAF frame deltas: it accumulates real elapsed seconds and steps `tickEconomy / tickEntities / tickCombatMovement / tickShipCombat / tickScouts` in fixed `1/60 s` sub-steps (each scaled by `BASE_GAME_SPEED × gameSpeed`). `pumpSim` runs from **both** the rAF loop (smooth when visible) **and** a 1 s `setInterval` — so the idle sim keeps running (ships move, economy produces, combat resolves) even when the tab is minimized and rAF is paused. A shared backlog consumes elapsed time exactly once and never loses it; a long gap drains over several pumps. Particle/weapon visuals stay frame-based (visible only). Pause (speed 0) discards elapsed time rather than banking it.
